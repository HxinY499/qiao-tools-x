...
export const toolRoutes: ToolRoute[] = [
  ...
  {
    key: ToolKey.SseJsonlParser,
    path: '/sse-jsonl-parser',
    title: 'SSE / JSONL 解析',
    subtitle: '粘贴 SSE 或 JSONL 数据，自动识别格式并逐条解析为格式化 JSON',
    icon: Layers,
    component: lazy(() => import('@/pages/sse-jsonl-parser')),
    category: 'dev',
    seo: {
      description:
        '免费在线 SSE / JSONL 解析工具，自动识别 Server-Sent Events 与 JSON Lines (NDJSON) 数据格式，逐条解析为格式化 JSON，支持语法高亮、逐条查看、错误定位与一键合并复制。',
      keywords: 'SSE解析,JSONL解析,JSON Lines,NDJSON,Server-Sent Events,SSE转JSON,流式数据解析,EventStream',
    },
  },
  {
